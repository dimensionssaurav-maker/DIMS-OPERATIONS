...
export const db = {
  ...
  purchaseOrders: {
    getAll: () =>
      supabase
        ...
        .select('*, supplier:suppliers(name, gst_no), items:purchase_order_items(*, material:materials(name, unit))')
  ...
  production: {
    ...
    update: (id: number, row: any) =>
      supabase.from('production').update({ ...row, updated_at: new Date().toISOString() }).eq('id', id).select().single(),
  },
