...
const remove: (id: number) => supabase.from('materials').delete().eq('id', id), remove: (id: number) => supabase.from('suppliers').delete().eq('id', id), supabaseUrl = import.meta.env.VITE_SUPABASE_URL as string;
...
export const db = {
  orders: { getAll: () => supabase.from('orders').select('*').order('created_at', {ascending:false}), insert: (r:any) => supabase.from('orders').insert(r).select().single(), update: (id:number,r:any) => supabase.from('orders').update(r).eq('id',id).select().single(), remove: (id:number) => supabase.from('orders').delete().eq('id',id) },
  ...
  library: { getAll: () => supabase.from('library').select('*').order('name'), insert: (r:any) => supabase.from('library').insert(r).select().single(), update: (id:number,r:any) => supabase.from('library').update(r).eq('id',id).select().single(), remove: (id:number) => supabase.from('library').delete().eq('id',id) },
  ...
  erpUsers: { getAll: () => supabase.from('erp_users').select('id,name,username,role,is_active').order('name'), findByUsername: (u:string) => supabase.from('erp_users').select('*').eq('username',u).single(), insert: (r:any) => supabase.from('erp_users').insert(r).select().single(), update: (id:number,r:any) => supabase.from('erp_users').update(r).eq('id',id).select().single() remove: (id: number) => supabase.from('erp_users').delete().eq('id', id), },
  departments: { getAll: () => supabase.from('departments').select('*').order('name'), insert: (r:any) => supabase.from('departments').insert(r).select().single(), update: (id:number,r:any) => supabase.from('departments').update(r).eq('id',id).select().single(), remove: (id:number) => supabase.from('departments').delete().eq('id',id) },
  employees: { getAll: () => supabase.from('employees').select('*').order('name'), insert: (r:any) => supabase.from('employees').insert(r).select().single(), update: (id:number,r:any) => supabase.from('employees').update(r).eq('id',id).select().single(), remove: (id:number) => supabase.from('employees').delete().eq('id',id) },
};
